...
object MailAllAbnormal {
  def main(args: Array[String]): Unit = {
    ...
    alertKafakValue.sinkTo(warningProducer).setParallelism(kafkaSinkParallelism)
    env.execute(jobName)
  ...
  class AbnormalProcess() extends ProcessFunction[MailModel, ((Object, Boolean), String)] {
    ...
    override def open(parameters: Configuration): Unit = {
      ...
      //mail的druid表名
      tableName = globConf.getString(ConfigOptions.key(Constants.DRUID_MAIL_TABLE_NAME).stringType().defaultValue(""))
