...
object DnsFlink {
  def main(args: Array[String]): Unit = {
    ...
    //该任务的名字
    val jobName = confProperties.getValue(Constants.DNS_FLINK_TO_DRUID_CONFIG, Constants.DNS_JOB_NAME)
    //kafka Source的名字
    val kafkaSourceName = confProperties.getValue(Constants.DNS_FLINK_TO_DRUID_CONFIG, Constants.DNS_KAFKA_SOURCE_NAME)
    //tranquility sink的名字
    val tranquilitySinkName = confProperties.getValue(Constants.DNS_FLINK_TO_DRUID_CONFIG, Constants
      .DNS_TRANQUILITY_SINK_NAME)
    //kafka sink的名字
    val kafkaSinkName = confProperties.getValue(Constants.DNS_FLINK_TO_DRUID_CONFIG, Constants.DNS_KAFKA_SINK_NAME)

    //话单处理的并行度
    val dealParallelism = confProperties.getIntValue(Constants.DNS_FLINK_TO_DRUID_CONFIG, Constants
      .DNS_DEAL_PARALLELISM)
    //check pointing的间隔
    val checkpointInterval = confProperties.getLongValue(Constants.DNS_FLINK_TO_DRUID_CONFIG, Constants
      .DNS_CHECKPOINT_INTERVAL)

    //该话单的topic
    val topic = confProperties.getValue(Constants.DNS_FLINK_TO_DRUID_CONFIG, Constants.DNS_TOPIC)
    //写入kafka的topic
    val toKafkaTopic = confProperties.getValue(Constants.DNS_FLINK_TO_DRUID_CONFIG, Constants.DNS_TO_KAFKA_TOPIC)
    ...
    //设置话单的表名
    conf("druid.dns.source") = tableName
    ...
    //将从kafka收到的数据包装成DnsModel
    val dnsModelStream = dStream
      .map(DnsModel.getDnsModel _).setParallelism(dealParallelism)
      .filter(_.isDefined).setParallelism(dealParallelism)
      .map(_.head).setParallelism(dealParallelism)


    //发送至druid
    dnsModelStream.addSink(new BeamSink[DnsModel](new DnsModelBeamFactory(conf)))
      .name(tranquilitySinkName).uid(tranquilitySinkName).setParallelism(1)

    //发送至kafka
    dnsModelStream.map(JsonUtil.toJson(_)).addSink(producer).uid(kafkaSinkName).name(kafkaSinkName).setParallelism(dealParallelism)
