...
class QuintetModelBeamFactory (conf: collection.mutable.HashMap[String,String]) extends BeamFactory[QuintetModel]{
  lazy val makeBeam: Beam[QuintetModel] = {
    ...
    val dataSource = conf("druid.quintet.source")
    val dimensions = QuintetModel.Columns
    val aggregators = QuintetModel.Metrics

    // Expects simpleEvent.timestamp to return a Joda DateTime object.
    DruidBeams
      .builder((quintetModel: QuintetModel) => new DateTime(quintetModel.timeStamp))
      .timestampSpec(new TimestampSpec("timeStamp","auto",null))
      .curator(curator)
      .discoveryPath(discoveryPath)
      .location(DruidLocation.create(indexService, dataSource))
      .rollup(DruidRollup(SpecificDruidDimensions(dimensions), aggregators, new PeriodGranularity(Period.parse("PT1S"), null, DateTimeZone.forID(QuintetModel.druidFormat))))
      .tuning(
        ClusteredBeamTuning
