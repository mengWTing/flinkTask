...
//    env.enableCheckpointing(checkpointInterval)
  ...
  class SqlInjectionProcessFunction extends ProcessFunction[(OperationModel, String), ((Object, Boolean), String)] {
    ...
    override def open(parameters: Configuration): Unit = {
      ...
      groupSplit = globConf.getInteger(ConfigOptions.key(Constants.SQL_INJECTION_GROUP_SPLIT).intType().defaultValue(0)).asInstanceOf[Char]

      kvSplit = globConf.getInteger(ConfigOptions.key(Constants.SQL_INJECTION_KV_SPLIT).intType().defaultValue(0)).asInstanceOf[Char]
