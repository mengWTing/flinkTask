...
public class Libinjection {
    ...
    public Libinjection(String path, String fileSystemType) {
        keywords = new Keyword(path, fileSystemType);  /* keyword hashmap */
    }
...
//		System.out.println(output);
    ...
    public int libinjection_sqli_fold() {
        ...
        Token last_comment = new Token(CHAR_NULL, 0, 0,
            null); /* A comment token to add additional info */
        ...
        while (true) {
            ...
            if (pos >= LIBINJECTION_SQLI_MAX_TOKENS) {
                if ((state.tokenvec[0].type == TYPE_NUMBER
                    ...
                    || state.tokenvec[1].type == TYPE_COMMA)
            ...
            if (state.tokenvec[left].type == TYPE_STRING
                ...
                continue;
            } else if (syntax_merge_words(state.tokenvec[left], left, state.tokenvec[left + 1],
                left + 1)) {
                pos -= 1;
                state.stats_folds += 1;
                if (left > 0) {
             ...
             */
                ...
                state.tokenvec[left].val.toUpperCase().equals("USER_ID") || state.tokenvec[left].val
                    .toUpperCase().equals("USER_NAME") ||
            ...
            if (state.tokenvec[left].type == TYPE_NUMBER
                ...
                left = 0;
                continue;
            } else if (state.tokenvec[left].type == TYPE_COMMA && token_is_unary_op(
                state.tokenvec[left + 1])
                && (state.tokenvec[left + 2].type == TYPE_NUMBER
                || state.tokenvec[left + 2].type == TYPE_BAREWORD
                || state.tokenvec[left + 2].type == TYPE_VARIABLE
                || state.tokenvec[left + 2].type == TYPE_STRING)) {
    ...
    public int parse_dash() {
        ...
        if (pos + 2 < slen && s.charAt(pos + 1) == '-' && char_is_white(s.charAt(pos + 2))) {
            ...
            return parse_eol_comment();
        } else if (pos + 1 < slen && s.charAt(pos + 1) == '-'
            && (state.flags & FLAG_SQL_ANSI) != 0) {
    ...
    public int parse_string_core(char delim, int offset) {
        ...
        while (true) {
            if (qpos == -1) {
                ...
                Token token = new Token(TYPE_STRING, pos + offset, slen - pos - offset,
                    s.substring(pos + offset));
    ...
    public int parse_qstring_core(int offset) {
        ...
        if (found == -1) {
            ...
            Token token = new Token(TYPE_STRING, pos + 3, found - pos - 3,
                s.substring(pos + 3, found));
            token.str_open = 'q';
            token.str_close = 'q';
            state.tokenvec[state.current] = token;
    ...
    public int parse_bword() {
        ...
        if (endptr == -1) {
            ...
            Token token = new Token(TYPE_BAREWORD, pos, endptr + 1 - pos,
                s.substring(pos, endptr + 1));
            state.tokenvec[state.current] = token;
            return endptr + 1;
        }
    }
        ...
        for (int i = 0; i < token.len; i++) {
            ...
            if (delim == '.' || delim == '`') {
                ...
                if (wordtype != null && wordtype != TYPE_NONE && wordtype != TYPE_BAREWORD) {
                     ...
                     */
                    state.tokenvec[state.current] = new Token(wordtype, pos, i,
                        word.substring(0, i));
                    return pos + i;
                }
            }
        ...
        if (xlen == 0) {
            if (s.charAt(pos + 1) == '$') {
                ...
                if (strend == -1) {
                    ...
                    Token token = new Token(TYPE_STRING, pos + 2, slen - (pos + 2),
                        s.substring(pos + 2));
                    token.str_open = '$';
                    token.str_close = CHAR_NULL;
                    state.tokenvec[state.current] = token;
                    ...
                    Token token = new Token(TYPE_STRING, pos + 2, strend - (pos + 2),
                        s.substring(pos + 2, strend));
                    token.str_open = '$';
                    token.str_close = '$';
                    state.tokenvec[state.current] = token;
                    return strend + 2;
                ...
                xlen = strlenspn(s.substring(pos + 1),
                ...
                if (xlen == 0) {
                    ...
                    if (strend == -1 || strend < pos + xlen + 2) {
                         ...
                         */
                        Token token = new Token(TYPE_STRING, pos + xlen + 2,
                            strend - pos - xlen - 2,
    ...
    public int parse_number() {
        ...
        if (s.charAt(pos) == '0' && pos + 1 < slen) {
            ...
            if (digits != null) {
                ...
                if (xlen == 0) {
                    ...
                    Token token = new Token(TYPE_NUMBER, pos, 2 + xlen,
                        s.substring(pos, pos + 1 + xlen + 1));
                    state.tokenvec[state.current] = token;
                    return pos + 1 + xlen + 1;
                }
            }
         ...
         */
        if (pos < slen
            && (s.charAt(pos) == 'd' || s.charAt(pos) == 'D' || s.charAt(pos) == 'f'
            || s.charAt(pos) == 'F')) {
            if (pos + 1 == slen) {
        ...
        if (have_e && !have_exp) {
             ...
             * number!!
             */
            state.tokenvec[state.current] = new Token(TYPE_BAREWORD, start, pos - start,
                s.substring(start, pos));
        } else {
            state.tokenvec[state.current] = new Token(TYPE_NUMBER, start, pos - start,
                s.substring(start, pos));
        }
        return pos;
    }
