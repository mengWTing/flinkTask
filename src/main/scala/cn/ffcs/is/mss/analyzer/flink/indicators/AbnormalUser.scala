...
import cn.ffcs.is.mss.analyzer.bean.{BbasExtremelyActiveUserEntity, BbasOperationPersonnelDownloadEntity, BbasSqlInjectionWarnEntity, BbasXssInjectionWarnEntity}
import cn.ffcs.is.mss.analyzer.druid.model.scala.OperationModel
import cn.ffcs.is.mss.analyzer.flink.sink.{DataSetKafkaSink, MySQLSink}
 ...
 * @Auther chenwei
 * @Description
 * @Date: Created in 2018/12/19 11:38
 * @Modified By
 */
object AbnormalUser {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //该任务的名字
    val jobName = confProperties.getValue(Constants.FLINK_ABNORMAL_USER_CONFIG, Constants
      .ABNORMAL_USER_JOB_NAME)
    //kafka Source的名字
    val kafkaSourceName = confProperties.getValue(Constants.FLINK_ABNORMAL_USER_CONFIG, Constants
      .ABNORMAL_USER_KAFKA_SOURCE_NAME)
    //mysql sink的名字
    val mysqlSinkName = confProperties.getValue(Constants.FLINK_ABNORMAL_USER_CONFIG, Constants
      .ABNORMAL_USER_MYSQL_SINK_NAME)
    //kafka sink 的名字
    val kafkaSinkName = confProperties.getValue(Constants.FLINK_ABNORMAL_USER_CONFIG, Constants
      .ABNORMAL_USER_KAFKA_SINK_NAME)

    //kafka Source的并行度
    val kafkaSourceParallelism = confProperties.getIntValue(Constants.FLINK_ABNORMAL_USER_CONFIG,
      Constants.ABNORMAL_USER_KAFKA_SOURCE_PARALLELISM)
    //kafka sink的并行度
    val kafkaSinkParallelism = confProperties.getIntValue(Constants.FLINK_ABNORMAL_USER_CONFIG,
      Constants.ABNORMAL_USER_KAFKA_SINK_PARALLELISM)

    //对数据处理的并行度
    val dealParallelism = confProperties.getIntValue(Constants.FLINK_ABNORMAL_USER_CONFIG,
      Constants.ABNORMAL_USER_DEAL_PARALLELISM)
    //写入mysql的并行度
    val mysqlSinkParallelism = confProperties.getIntValue(Constants.FLINK_ABNORMAL_USER_CONFIG,
      Constants.ABNORMAL_USER_MYSQL_SINK_PARALLELISM)

    //kafka的服务地址
    val brokerList = confProperties.getValue(Constants.FLINK_COMMON_CONFIG, Constants
      .KAFKA_BOOTSTRAP_SERVERS)
    //flink消费的group.id
    val groupId = confProperties.getValue(Constants.FLINK_ABNORMAL_USER_CONFIG, Constants
      .ABNORMAL_USER_GROUP_ID)
    //kafka source 的topic
    val sourceTopic = confProperties.getValue(Constants.OPERATION_FLINK_TO_DRUID_CONFIG, Constants.OPERATION_TO_KAFKA_TOPIC)
    //kafka sink 的 topic
    val sinkTopic = confProperties.getValue(Constants.FLINK_ABNORMAL_USER_CONFIG, Constants.ABNORMAL_USER_KAFKA_SINK_TOPIC)
    ...
    parameters.setInteger(Constants.ABNORMAL_USER_TOP_K, confProperties.getIntValue(Constants
      .FLINK_ABNORMAL_USER_CONFIG, Constants.ABNORMAL_USER_TOP_K))


    //check pointing的间隔
    val checkpointInterval = confProperties.getLongValue(Constants.FLINK_ABNORMAL_USER_CONFIG,
      Constants.ABNORMAL_USER_CHECKPOINT_INTERVAL)

    //
    val waterMark = confProperties.getLongValue(Constants.FLINK_ABNORMAL_USER_CONFIG, Constants
      .ABNORMAL_USER_WATERMARK)
    //聚合窗口大小
    val windowSize = confProperties.getLongValue(Constants.FLINK_ABNORMAL_USER_CONFIG, Constants
      .ABNORMAL_USER_WINDOW_SIZE)
    //聚合滑动窗口大小
    val slideSize = confProperties.getLongValue(Constants.FLINK_ABNORMAL_USER_CONFIG, Constants
      .ABNORMAL_USER_SLIDE_SIZE)
    ...
    val abnormalUserStream = dStream
      .map(tuple => {
      ...
      .process(new AbnormalUserProcessFunction)

    abnormalUserStream.addSink(new MySQLSink)
      .uid(mysqlSinkName)
      .name(mysqlSinkName)
      .setParallelism(mysqlSinkParallelism)


    abnormalUserStream.map(o => {
      JsonUtil.toJson(o._1.asInstanceOf[BbasExtremelyActiveUserEntity])
    })
      .addSink(producer)
      .uid(kafkaSinkName)
      .name(kafkaSinkName)
  ...
  class AbnormalUserProcessFunction extends ProcessFunction[Long, (Object, Boolean)] {
    ...
    override def open(parameters: Configuration): Unit = {
      ...
      classMap.put(classOf[BbasSqlInjectionWarnEntity], ("WARN_DATETIME", "USERNAME"))
      classMap.put(classOf[BbasXssInjectionWarnEntity], ("WARN_DATETIME", "USERNAME"))
      classMap.put(classOf[BbasOperationPersonnelDownloadEntity], ("WARN_DATETIME", "USERNAME"))
      K = globConf.getInteger(Constants.ABNORMAL_USER_TOP_K, 10)
    ...
    override def processElement(value: Long, ctx: ProcessFunction[Long, (Object, Boolean)
      ...
      val abnormalUserMap = mutable.Map[String, Double]()
      for ((clazz, (whereFiledName, userNameFiledName)) <- classMap) {

        val list = sqlHelper.query(clazz, getWhereSql(time, whereFiledName))
        if (list != null && list.size() > 0) {
          for (o <- list) {
            val userName = o.getV(userNameFiledName).toString
            if ("匿名用户".equals(userName)) {
              abnormalUserMap.put(userName, abnormalUserMap.getOrElse(userName, 0.0) + K)
            }
      ...
      if (abnormalUserMap.size < K) {

        val list = DruidUtil.query(getEntity(time, K, tableName))
        for (i <- 0 until list.size()) {
          val userName = list.get(i).get(Dimension.userName.toString)
          abnormalUserMap.put(userName, abnormalUserMap.getOrElse(userName, 0.0) + (list.size() - i))
        }

      }


      abnormalUserMap.toList.sortBy(_._2).take(10).foreach(tuple => {

        val userName = tuple._1
        val value = tuple._2

        val bbasExtremelyActiveUserEntity = new BbasExtremelyActiveUserEntity
        bbasExtremelyActiveUserEntity.setUsername(userName)
        bbasExtremelyActiveUserEntity.setActiveDatetime(new Timestamp(time - TimeUtil.MINUTE_MILLISECOND))
        bbasExtremelyActiveUserEntity.setInformation(value.toString)
//        bbasExtremelyActiveUserEntity.setSourceIp()
//        bbasExtremelyActiveUserEntity.setDestIp()
        out.collect((bbasExtremelyActiveUserEntity, true))
      })
    ...
    def getEntity(time: Long, limit: Int, tableName: String): Entity = {
      ...
      entity.setLimitSpec(LimitSpec.getLimitSpec(Aggregation.connCount, LimitSpec.Direction.descending, LimitSpec.DimensionOrder.numeric, limit))
      ...
      entity.setFilter(Filter.getFilter(Filter.not, Filter.getFilter(Filter.selector, Dimension.userName, "匿名用户")))
      entity
    }
