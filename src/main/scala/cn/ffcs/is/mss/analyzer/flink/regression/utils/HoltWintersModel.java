...
public class HoltWintersModel {
    ...
    public static Double getbetterforecast(Double[] y, int period, int m) {
     ...
     * @return
     */
    public static double[] betterforecast(Double[] y, int period, int m) {
        double[] result = null;
        double avg = 0;
        long len = alphas.length;
        for (int i = 0; i < len; i++) {
            int sum = 0;
            double[] temp = forecast(y, alphas[i], 0.25, 0, period, m, false);
            for (int j = 0; j < y.length; j++) {
                sum += Math.abs(y[j] - temp[j]);
            }
            if (i == 0) {
                avg = sum / y.length;
                result = temp;
...
//        long time = System.currentTimeMillis();
        ...
        for (int i = 2; i < length; i++) {
            ...
            if (debug) {
                ...
                System.out.println("" + y[i] + "|" + St[i] + "|" + Bt[i] + "|" + It[i] + "|" + Ft[i]);
            }
...
//        System.out.println(System.currentTimeMillis() - time);
     ...
     * @return
     */
    public static double[] betterSec(Double[] y, int m) {
        double[] result = null;
        double avg = 0;
        long len = alphas.length;
        for (int i = 0; i < len; i++) {
     ...
     * @return 预测值
     */
    public static double[] getSecExpect(Double[] list, int year, Double modulus) {
        if (list.length < 20 || modulus <= 0 || modulus >= 1) {
            return null;
        }
        double[] ft = new double[list.length + year];
        Double modulusLeft = 1 - modulus;
        // 一次预测 >20项默认第一个值为初始值
        Double lastIndex = Double.parseDouble(list[0].toString());
        // 二次预测 >20项默认第一个值为初始值
        Double lastSecIndex = Double.parseDouble(list[0].toString());
     ...
     * @return 预测值
     */
    public static Double getThridExpect(Double[] list, int year, Double modulus) {
        if (list.length < 20 || modulus <= 0 || modulus >= 1) {
            return null;
        }
        Double modulusLeft = 1 - modulus;
        // 一次预测 >20项默认第一个值为初始值
        Double lastIndex = (double) ((list[0] + list[1] + list[2]) / 3);
        // 二次预测 >20项默认第一个值为初始值
