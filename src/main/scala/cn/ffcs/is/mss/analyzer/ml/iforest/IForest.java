package cn.ffcs.is.mss.analyzer.ml.iforest;
 ...
 * @Auther chenwei
 * @Description
 * @Date: Created in 2017/11/6 下午5:06
 * @Modified By
 */
public class IForest implements Serializable {
     ...
     * @return
     */
    public double getScore(double[][] samples, int t, int subSampleSize, int n) throws Exception {
        this.subSampleSize = subSampleSize;
     ...
     * @return
     */
    public double getScore2(double[][] samples, int t, int subSampleSize, int n) throws Exception {
        this.subSampleSize = subSampleSize;
     ...
     * @return
     */
    public double[] getScores(double[][] samples, int t, int subSampleSize) throws Exception{
        this.subSampleSize = subSampleSize;
        ...
        // 第一步：创建Isolation Forest
        createIForest(samples, t);
        return computeAnomalyIndex(samples);
    }


    /**
     * 训练模型，并返回指定样本的异常情况，正常为1，异常为0
     * @param samples
     * @param t
     * @param sample
     * @param needFilter
     ...
     * @throws Exception
     */
    public boolean predict(double[][] samples, int t, double[] sample,boolean needFilter, boolean filterBigger) throws Exception {
        return predict(samples,t,256,100,sample,needFilter,filterBigger);
    }

    /**
     * 训练模型，并返回指定样本的异常情况，正常为true，异常为false
     * @param samples
     * @param t
     * @param subSampleSize
     * @param iters
     ...
     * @throws Exception
     */
    public boolean predict(double[][] samples, int t, int subSampleSize, int iters, double[] sample,
        boolean needFilter, boolean filterBigger)
        throws Exception {
        ...
        if (needFilter) {
            ...
            //计算每一列的平均值
            double[] means = new double[sample.length];
            ...
            //如果需要过滤比平均值大的
            if (filterBigger) {
                //存在比平均值小的特征则进行异常检测，不然返回1，认为是正常的
                for (int i = 0; i < sample.length; i++) {
                    if (sample[i] < means[i]) {
                        break outer;
                    }
                ...
                //存在比平均值大的特征则进行异常检测，不然返回1，认为是正常的
                for (int i = 0; i < sample.length; i++) {
                    if (sample[i] > means[i]) {
                        break outer;
                    }
        ...
        train(samples, t, subSampleSize, iters);
    ...
    /**
     * 预测样本 sample 是否为异常值，正常返回true，异常返回false
     * @param sample
     * @return
     * @throws Exception
     */
    public boolean predict(double[] sample) throws Exception {
        double score = computeAnomalyIndex(sample);
        double dis0 = Math.abs(score - center0);
        double dis1 = Math.abs(score - center1);
